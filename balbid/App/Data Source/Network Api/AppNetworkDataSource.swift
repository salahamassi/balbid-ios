...
final class AppNetworkApiDataSource: DataSource {
    ...
    func handleData<T: SwiftyModelData>(_ data: Data?, _ error: WinchError?, _ type: T.Type, for service: NetworkApiDataSourceService, completion: BalbildDataSourceCompletion<T>?) {
        if let data = data{
    ...
    private func getHeaders(mustUseAuth: Bool) -> [String: String] {
        ...
        headers = [
            "Accept-Language": UserDefaultsManager.appLanguage ?? "en",
